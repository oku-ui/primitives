                ...
                default: () => h('OkuMenuContentImpl',
                  {
                    ...
                    'onFocusOutside': composeEventHandlers<MenuSubContentEmits['focusOutside'][0]>((event: FocusEvent) => {
                      // We prevent closing when the trigger is focused to avoid triggering a re-open animation
                      // on pointer interaction.
                      if (event.target !== subInject.trigger.value)
                        inject.onOpenChange(false)
                    }),
                    'onEscapeKeydown': composeEventHandlers<MenuSubContentEmits['keydown'][0]>((event: KeyboardEvent) => {
                      rootInject.onClose()
                      // ensure pressing escape in submenu doesn't escape full screen mode
                      event.preventDefault()
                    }),
                    'onKeydown': composeEventHandlers<MenuSubContentEmits['keydown'][0]>((event: KeyboardEvent) => {
                      // Submenu key events bubble through portals. We only care about keys in this menu.
                      const isKeyDownInside = event.valueTarget.contains(event.target as HTMLElement)
