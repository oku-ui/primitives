import { defineComponent, h, onBeforeUnmount, ref, toRefs, watchEffect } from 'vue'
...
import { MENU_SUB_TRIGGER_NAME, SUB_OPEN_KEYS, scopedMenuProps, useMenuContentInject, useMenuInject, useMenuRootInject, useMenuSubInject } from './props'
        ...
        default: () => h(OkuMenuItemImpl,
          {
             ...
             */
              event.valueTarget.focus()
              if (!inject.open.value)
                inject.onOpenChange(true)
            },
            'onPointermove': composeEventHandlers<MenuSubTriggerEmits['pointermove'][0]>(whenMouse((event: PointerEvent) => {
            ...
            'onPointerleave': composeEventHandlers<MenuSubTriggerEmits['pointerleave'][0]>(whenMouse((event: PointerEvent) => {
              clearOpenTimer()
