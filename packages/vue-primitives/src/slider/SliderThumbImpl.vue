...
import { Collection, useSliderContext } from './Slider.ts'
...
const value = computed(() => context.values.value[props.index])

const percent = computed(() => value.value === undefined ? 0 : convertValueToPercentage(value.value, context.min.value, context.max.value))
const label = computed(() => getLabel(props.index, context.values.value.length))

const thumbInBoundsOffset = computed(() => {
  const orientationSize = size.value?.[orientation.size]
  return orientationSize ? getThumbInBoundsOffset(orientationSize, percent.value, orientation.direction) : 0
})
...
const onFocus = composeEventHandlers((event) => {
  ...
  context.valueIndexToChangeRef.value = props.index
})
