...
import { ScopePropObject, createProvideScope } from '.'
...
describe('Provide', () => {
  ...
  test('createProvideScope scopeName', async () => {
    // Collection
    const PROVIDER_NAME = 'TestCollectionProvider'
    ...
    const useCreationScope = createCollectionScope()
    ...
    const AvatarFallback = {
      ...
      setup(props) {
        const scopes = useCreationScope(props.scopeOkuAvatar)
        console.log('a', scopes)
        const inject = useAvatarInject('AvatarFallback', props.scopeOkuAvatar)
        return () => h(Collection, {
          ...scopes,
        }, {
          default: () => (inject as AvatarProvideValue).imageLoadingStatus,
        })
      },
    } as Component
